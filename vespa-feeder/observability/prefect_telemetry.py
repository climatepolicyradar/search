...
def get_flow_context() -> FlowContext:
    ...
    try:
        ...
        deployment_name = getattr(prefect.runtime.deployment, "name", None)
        deployment_id = getattr(prefect.runtime.deployment, "id", None)
        return FlowContext(
        ...
        return FlowContext(flow_name=None, deployment_name=None, deployment_id=None)
...
class PrefectContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        """Inject Prefect flow/task run context fields into the log record."""
        flow_context = get_flow_context()
        setattr(record, "flow_name", flow_context.flow_name)
        setattr(record, "deployment_name", flow_context.deployment_name)
        setattr(record, "deployment_id", flow_context.deployment_id)
...
class PrefectLogContextProcessor(BatchLogRecordProcessor):
    ...
    def on_emit(self, log_record: ReadWriteLogRecord) -> None:
        """Enrich the OTEL log record with Prefect context before export."""
        try:
            ...
            deployment_name = flow_context.deployment_name
            if deployment_name is not None and record.attributes is not None:
                record.attributes["deployment_name"] = deployment_name  # type: ignore[index]
            deployment_id = flow_context.deployment_id
            if deployment_id is not None and record.attributes is not None:
                record.attributes["deployment_id"] = deployment_id  # type: ignore[index]
...
class PrefectSpanContextProcessor(SpanProcessor):
    ...
    def on_start(self, span: Span, parent_context: Optional[Context] = None) -> None:  # noqa: ARG002
        """Set Prefect flow/task context attributes on the span when it starts."""
        try:
            flow_context = get_flow_context()
            flow_name = flow_context.flow_name
    ...
    def on_end(self, span: ReadableSpan) -> None:
        """No-op: span enrichment happens on start, not end."""
        pass
