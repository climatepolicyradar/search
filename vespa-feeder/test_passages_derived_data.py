...
`passages_derived_data` itself is - see that module's docstring. Kept verbatim
apart from calling the detectors with `text` / `page_numbers` instead of a
`Passage`, so the files stay diffable: **change both, or neither**.
...
def test_legal_footnote_block_is_a_reference_list():
    assert looks_like_reference_list(
        LEGAL_FOOTNOTES_WITH_NO_SINGLE_STRONG_SIGNAL, "footnote"
    )
...
def test_e_signature_block_typed_as_footnote_is_not_a_reference_list():
    ...
    assert not looks_like_reference_list(text, content_type)
...
def test_whether_reference_lists_are_detected(text, content_type):
    assert looks_like_reference_list(text, content_type)
