...
// as `(page_token - 1) * page_size` (search/engines/dev_vespa.py). Unlike
// documents/passages, navigator-frontend always requests labels with
// `page_size=10000` in one shot (src/hooks/useLabelSearch.ts) rather than
// paging — these combinations still cover first page, a deep page (tests
// the cost of Vespa skipping over ranked results internally), and a large
// `page_size`, using a broad query (no `query` param — matches all labels)
// so every case returns full pages against the fixed labels dataset (~few
// hundred distinct labels at time of writing).
...
const paginationCombinations = new SharedArray(
  ...
  function (): TPaginationCombination[] {
    return [
      ...
      {
        name: "large page_size (real navigator-frontend shape)",
        pageToken: 1,
        pageSize: 1000,
        verifyOffsetAdvances: false,
      },
...
export const options = resolveProfile(
  "labels: pagination combinations",
  PROFILES,
);
...
export default function () {
  ...
  const res = http.get(
    `${BASE_URL}/labels?page_token=${combination.pageToken}&page_size=${combination.pageSize}`,
    {
      // Group by route path + the relevant query param *names* (never
      // values) instead of letting k6 default `name`/`url` to the full
      // dynamic query string — per-request page_token/page_size values were
      ...
      tags: { name: "labels?page_token,page_size" },
    },
  );
  ...
  check(res, {
    [`${combination.name}: status is 200`]: (response: Response) =>
      response.status === 200,
    [`${combination.name}: returns at most page_size results`]: (
      response: Response,
    ) => {
      if (response.status !== 200) return false;
      const body = response.json() as TSearchResponse;
      const results = body?.results ?? [];
      // "at most" rather than "exactly": unlike documents' large
      // ~17k-result query fixture, the labels dataset is small (a few
      // hundred distinct labels), so a page_size of 1000 legitimately
      // returns fewer than page_size once the dataset is exhausted.
      return (
        results.length > 0 &&
  ...
  if (combination.verifyOffsetAdvances) {
    ...
    // a deep page must return different labels than page 1.
    const firstPageRes = http.get(
      `${BASE_URL}/labels?page_token=1&page_size=${combination.pageSize}`,
      { tags: { name: "labels?page_token,page_size" } },
    );
    check(firstPageRes, {
