...
export default function () {
  ...
  const res = http.get(
    `${BASE_URL}/documents?query=${encodeURIComponent(query)}${fieldsQuery ? `&${fieldsQuery}` : ""}${filtersQuery}${cacheBuster}`,
    {
      ...
      // dynamic query string, fields list, filters JSON, and cache-buster —
      // that was producing a high-cardinality set of unique values across
      // http_reqs, http_req_waiting, and http_req_tls_handshaking (flagged
      // by Cloud Insights' Metric Tags audit). Naming convention across this
