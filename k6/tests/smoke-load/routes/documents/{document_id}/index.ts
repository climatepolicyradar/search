...
export default function () {
  ...
  const res = http.get(`${BASE_URL}/documents/${documentId}${cacheBuster}`, {
    ...
    // the path param name in place of a value — no query params here to
    // list) instead of letting k6 default `name`/`url` to the full dynamic
    // path + cache-buster — per-request document_id and cache-buster values
    // were producing a high-cardinality set of unique values across
    // http_reqs, http_req_waiting, and http_req_tls_handshaking (flagged by
    // Cloud Insights' Metric Tags audit). Naming convention across this
