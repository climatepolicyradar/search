...
const PROFILES = {
  ...
  load: {
    ...
    // search/engines/dev_vespa/passages_search_engine.py) — no fan-out, unlike
...
export default function () {
  ...
  const res = http.get(
    `${BASE_URL}/passages?query=${encodeURIComponent(query)}${cacheBuster}`,
    {
      ...
      // query text and cache-buster values were producing a high-cardinality
      ...
      tags: { name: "passages?query", url: "passages?query", query },
    },
  );
