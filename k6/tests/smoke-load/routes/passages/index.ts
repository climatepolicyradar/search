...
export default function () {
  ...
  const res = http.get(
    `${BASE_URL}/passages?query=${encodeURIComponent(query)}${cacheBuster}`,
    {
      ...
      // the full dynamic query string plus load-mode cache buster —
      // per-request query text and cache-buster values were producing a
      ...
      // route into one series regardless of search term, profile, or
      // cache-buster value.
      // https://grafana.com/docs/k6/latest/using-k6/http-requests/#url-grouping
      tags: { name: "passages:base query" },
    },
  );
