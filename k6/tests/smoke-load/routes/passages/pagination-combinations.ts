...
// (search/engines/dev_vespa/passages_search_engine.py, same pagination logic
// as /documents). Covers the first page (default), a deep
...
export default function () {
  ...
  const res = http.get(
    `${BASE_URL}/passages?query=climate&page_token=${combination.pageToken}&page_size=${combination.pageSize}`,
    {
      ...
      tags: { name: "passages?query,page_token,page_size" },
    },
  );
