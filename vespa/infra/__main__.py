...
allowed_cidrs_by_name = config.require_object("allowed_cidrs")

# Normalize CIDRs to ensure they have netmask (e.g. /32) for AWS
for name, cidr in allowed_cidrs_by_name.items():
    try:
        allowed_cidrs_by_name[name] = str(ipaddress.ip_network(cidr))
    except ValueError:
        # Fallback for truly invalid or masked secrets in preview
        if pulumi.runtime.is_dry_run():
            allowed_cidrs_by_name[name] = "0.0.0.0/0"
        else:
            raise
    ...
    ingress=[
        {
            ...
            "description": "Access for vespa-dev LoadBalancer",
        }
    ],
    egress=[
...
docker run -d --name vespa -p 8080:8080 -p 19071:19071 \
  --hostname vespa-dev \
  -e VESPA_CONFIGSERVERS=vespa-dev \
  vespaengine/vespa
...
public_integration = aws.apigatewayv2.Integration(
    ...
    request_parameters={
        "overwrite:path": "/search/$request.path.proxy",
    },
)
