...
# region Extract
def extract_huggingface_data() -> dict[str, list[TextBlock]]:
    ...
    conn = duckdb.connect(str(DATA_CACHE_DIR / "temp.duckdb"))
    ...
    if not PASSAGES_CACHE_FILE.exists():
        (
            ...
            .agg(pl.struct(["type", "index", "text"]).alias("text_blocks"))
            .collect()
...
def write_updates_file(
    api_documents: list[Document], passages_map: dict[str, list[TextBlock]]
):
    BATCH_SIZE = 5000
    with OUTPUT_FILE.open("wb") as f:
        ...
        for document in api_documents:
            ...
            passages = passages_map.get(document_id, [])
            passage_objects: list[VespaPassage] = [
                {"text": p["text"], "language": "en"} for p in passages
            ]
            ...
            update_op: VespaUpdateOp = {
                ...
                "fields": {
                    ...
                    "source": orjson.dumps(
                        document | {"passages": passage_objects}  # type: ignore
                    ).decode(),
                },
            }
