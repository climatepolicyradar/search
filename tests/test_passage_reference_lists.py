...
def test_legal_footnote_block_is_a_reference_list():
    assert looks_like_reference_list(
        Passage(text=LEGAL_FOOTNOTES_WITH_NO_SINGLE_STRONG_SIGNAL)
    )
...
def test_whether_reference_lists_are_detected(text):
    assert looks_like_reference_list(Passage(text=text))
...
def test_whether_non_reference_lists_are_not_detected(text):
    assert not looks_like_reference_list(Passage(text=text))
