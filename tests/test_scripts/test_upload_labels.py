...
def test_whether_upload_labels_creates_files(
    ...
    with prefect_test_harness():
        with (
            ...
            patch(
                "scripts.data_uploaders.upload_labels.LABELS_PATH_STEM",
                labels_path_stem,
            ),
            patch(
            ...
            patch(
                "scripts.data_uploaders.upload_labels.upload_file_to_s3"
            ) as mock_upload,
        ):
            upload_labels = importlib.import_module(
                "scripts.data_uploaders.upload_labels"
            )
