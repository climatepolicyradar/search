...
def test_label_multi_word_subphrase_queries_surface_same_topic(
    vespa_app: Vespa, query: str
):
    """
    Multi-word label value query must surface the right topic.

    A multi-word label value must match single-token, sub-phrase,
    full-phrase, and prefix-style queries.
    """
    anchor_id = "concept::Qe2eAirPollutionRisk"
    _feed_label(
        vespa_app,
        VespaLabel(
            id=anchor_id,
            type="topic",
            value="air pollution risk",
            alternative_labels=[],
            subconcept_labels=[],
            description="",
            negative_labels=[],
